...
- **Flexible User Management** - Enhanced APIs for custom authentication schemes
  - `webAddUser()` now allows null password for apps managing their own authentication
  - `webLogin()` auto-creates users if not found in auth database (for custom user stores)
  - Auth functions available without `ME_WEB_HTTP_AUTH` for broader applicability
  - `requireTlsForBasic` now defaults to true for improved security
...
- **Flexible Builder Endpoint** - `cmdBuilder` configuration now available when `SERVICES_REGISTER` is defined (not just `SERVICES_CLOUD`)
  - Allows builder endpoint customization for provision-only builds
  - `SERVICES_REGISTER` automatically enabled when `SERVICES_PROVISION` or `SERVICES_CLOUD` is set

### Standalone Web Server
- **Web Server Only Mode** - The `web` program can now be run standalone without the full Ioto agent
  - Useful for local management without cloud-based management or embedded database
  - Documented in README with configuration instructions
...
- **Guard Page Auto-Growing Stacks** - New `ME_FIBER_GROWABLE_STACK` feature that uses virtual memory guard pages for automatic stack growth
  ...
  - Async-signal-safe stack growth handler (no logging in signal context)
...
- **Fixed URL connection reuse** - Host/port values now saved before parsing to enable proper connection reuse
...
- **Fixed rResumeWaitFiber** - Now passes actual event mask instead of hardcoded value
- **Fixed select memset sizes** - Corrected memset using wrong sizes for readEvents/writeEvents
- **Fixed highestFd optimization** - Select handler now searches backward for efficiency
- **Fixed duplicated condition** - Removed redundant check in `invokeExpired`
...
- Fixed type signedness warnings in watch iteration loops (`rWatchOff`, `rSignal`, `rSignalSync`)
- Moved variable declarations to top of functions for C89 compatibility
...
- Added error logging when fd exceeds FD_SETSIZE in epoll, kqueue, and select handlers
- Changed poll table growth failure from debug to error level for better visibility
...
### FreeRTOS Fiber Implementation
- **Semaphore-Based Synchronization** - Rewrote FreeRTOS context switching with mutex + condition semaphore pairs
  for robust cooperative fiber scheduling
- **Predicate-Based Wait Functions** - Added `resumed_or_done` and `resumed` predicates with spurious wakeup
  protection
- **Improved Task Lifecycle** - Better fiber creation and termination management with proper cleanup
- **Flexible Header Paths** - Support for both `freertos/FreeRTOS.h` and `FreeRTOS.h` include paths

### ESP32 Support
- **ESP32-C6 Support** - Added support for ESP32-C6 (RISC-V based) devices
- **Improved RISC-V Detection** - Fixed architecture detection using `__riscv && (__riscv_xlen == 32/64)`
- **Stack Configuration** - Integration with `CONFIG_ESP_MAIN_TASK_STACK_SIZE` from ESP-IDF sdkconfig
- **NULL Stack Support** - `uctx_setstack()` now allows NULL stack for platforms that manage stacks internally

### New Demo Applications
- **FreeRTOS Demo** - New `apps/demo/freertos/` with complete FreeRTOS integration example
  - Includes FreeRTOSConfig.h, Makefile, and main.c
  - Demonstrates Ioto integration with FreeRTOS POSIX/GCC demo

### OS Abstraction Improvements
- **OS Type Constants** - Added `ME_OS_*` constants to osdep.h for compile-time OS detection
  - ME_OS_MACOSX, ME_OS_LINUX, ME_OS_FREEBSD, ME_OS_WINDOWS, ME_OS_FREERTOS, etc.
- **Platform-Specific Stack Defaults** - 64KB for 64-bit systems, 32KB for 32-bit systems
- **Reduced Minimum Stack** - `UCTX_MIN_STACK_SIZE` reduced from 32KB to 8KB for embedded systems

### Build Documentation
- Simplified README-FREERTOS.md with streamlined build workflow
- Simplified README-ESP32.md with step-by-step instructions
- Added "Getting FreeRTOS" and "Getting ESP-IDF" sections
...
- ESP32 (ESP-IDF) including ESP32-S3 and ESP32-C6
- VxWorks
